...
export const CAPTIONS_EN = {
  ...
  '341-衆議院の議席が決まるまで': 'Each voter casts two ballots, one for a candidate in a single-member district and one for a party list, and the two are counted separately into 289 and 176 seats. They are not sealed off from each other: a party may enter the same person in both, so a candidate beaten in the district can still be revived from the list — and among equally ranked names, the closer the defeat, the better the chance.',
  ...
  '317-オーケストラの配置と音量': 'The strings run left to right from high to low and the louder the instrument the further back it sits, so the seating plan is itself a balance control — and "double wind" or "triple wind" names the size of the orchestra by counting only the woodwind',
  ...
  '310-ギターのフレット位置は等比': 'Frets crowd together as you climb because what is divided into twelve is not a distance but a ratio: each semitone multiplies by 2^(1/12), so the twelfth fret lands at exactly half the string and everything beyond it repeats at half size',
  '352-網点で濃さを出す': 'A press cannot print grey: it lays ink or it does not. Halftone varies the size of evenly spaced dots and lets the eye mix them. A shaded sphere at three screen rulings, round dots touching at 78.5 per cent, where round, square and elliptical dots join, rulings from newspapers (85 lpi) to fine art (300), and why the inks sit 30 degrees apart: screens 7 degrees apart beat into moiré.',
  '351-宇宙は無重力ではない': 'At the altitude of the ISS, 400km, Earth still pulls at 88.5 per cent of its surface strength. Astronauts float because the station is falling, not because gravity has gone: in one second it travels 7.66km and drops 4.34m, and 7.66km further on the surface has curved away by the same 4.34m. Tables give the pull at every altitude up to the Moon, and why it is called microgravity rather than zero.',
  '350-トロンボーンのポジション': 'The seven trombone slide positions at real size. They are not evenly spaced: 85mm from first to second, 113mm from sixth to seventh, each gap 1.0595 times the last — the twelfth root of two that spaces guitar frets, running outward instead of inward, because a slide lengthens the whole tube. Below: the partials at each position, and why an F attachment still cannot reach B1.',
  '349-等角図をzで組む': 'A cube in isometric projection whose source contains no paper coordinates at all — only x, y and z. The drawing engine applies the projection: a hundred along x moves 86.6 right and 50 down, y the same to the left, z straight up. The three edges hidden behind the solid are dashed because the author said so; the machine does not decide what is hidden.',
  '348-クローンは同じ個体ではない': 'CC, the first cloned cat, carried the same nuclear DNA as Rainbow and came out a different colour. Only the top row of the table comes from the original animal. The tortoiseshell pattern comes from neither cat: in every female one X chromosome is silenced at random, cell by cell, after fertilisation. It is not written in the DNA, so a clone cannot copy it.',
  '347-遺伝子組換えとゲノム編集': 'Both alter a genome; they differ in what is left behind. Recombination inserts a gene from another organism and that gene stays. Editing of the cut-and-leave kind severs the organism own sequence and lets the cell repair it, leaving a scar indistinguishable from a natural mutation. In Japan that structural difference is the legal one.',
  '346-尺とフィート': 'The Japanese shaku is 303.03 mm and the foot is 304.80 mm — both about a foot long, and neither the same. Lining them up at the left shows the right ends parting by 1.77 mm, a gap too thin to shade at full size, so the last 8 mm is enlarged twenty times beside it; stack thirty and the drawing is out by 53 mm.',
  '345-日本史の時代区分': 'A timeline of Japanese historical periods that refuses to be a single line: the Warring States era sits inside the Muromachi period for 106 years, two imperial courts ran in parallel for 56, and the two famous boundaries — the founding of the Kamakura shogunate and the turn from Edo to Meiji — are drawn as hatched bands of years rather than as a date, then enlarged below.',
  '344-接地工事の4種類': 'Three of the four Japanese earthing classes are given as a number — ten, ten and a hundred ohms — but class B, bonded to the neutral of a transformer bridging high and low voltage, is given as a formula: 150 divided by the earth fault current. Fix one number instead and the same voltage appears whatever the current.',
  ...
  '343-ミキサーの信号の道': 'The same control means different things depending on where in the chain it sits. An aux send taken before the fader keeps its level when the engineer moves the fader, which is what a performer monitor needs; taken after, it follows the fader, which is what a reverb needs. Insert is in series and cuts the path open; aux is in parallel and branches away and back.',
