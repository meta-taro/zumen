 ...
 */
/**
 * ## 120px まで下げてみて、戻した（2026-09-18）
 *
 * ビリヤードの図（見本 168）で、**150px の見出し欄から名前が飛んだ**のに黙っていた。
 * 下限を 120px にすると確かに拾えたが、**見本 30 のクレーンも鳴った** ——
 * あれは 120px の印の上に名前が出ているだけで、**読めていて、正しい置き方**。
 *
 * **鳴りっぱなしの指摘は読まれなくなる**（`tooSmallToProject` と同じ轍）。
 * 表の欄と小さい印を幅だけで見分けられない以上、ここは 200px のままにする。
 * 150px の欄は、**紙の上の文字の重なり**（`overlappingInk`）で拾えることがある。
 */
