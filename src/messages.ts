...
const ja = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**戻る線があるなら、groups を付けないほうが読めることが多い**（構成図。2026-09-20 に測った）。差し戻し・再申請・分割のように**前の節へ帰る線**があると、囲みをまたぐぶん経路が大回りして、紙が伸び交差が生まれる。特許の手続（15 節 17 辺）で測ると、**groups あり 1,773×1,224px・交差 1 → groups なし 1,455×700px・交差 0**。囲みは「同時に在るもの」（VPC・階・区域）に効き、**「順に進むもの」には効きにくい。**',
...
const en: Catalog = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**If the graph has edges that go back, try dropping groups** (structure diagrams; measured 2026-09-20). Rework, resubmission and division send lines back to earlier nodes, and routing them around container borders stretches the paper and creates crossings. On the Japanese patent procedure (15 nodes, 17 edges): with groups 1,773x1,224px and one crossing; without groups 1,455x700px and none. Containers help for things that exist at once (a VPC, a floor, a zone), not for things that happen in order.',
