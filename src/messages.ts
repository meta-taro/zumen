...
const ja = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**辺にラベルを書くなら、まず direction: down を試す**（構成図。ラベルのある 25 枚で測った・2026-09-20。**2026-09-21 に数え直したら 38 枚に増えている** —— 結論のほうは測り直していない）。**1 つの節へ線が集まると、right では入らないラベルが出る** —— 25 枚のうち down がラベルを増やしたことは **0 回**、減らしたのが 2 回（患者の動線 1 → 0、特許の手続 3 → 0）。ただし**紙は 23/25 で right のほうが小さい**ので、ラベルが全部出ているなら right でよい。**審級・組織・系統のように「上下」がある図は down、工程のように「前後」がある図は right** が読みやすい。',
      ...
      '**構成図で「戻る辺」を書くと、並びの順が崩れる。** 機械は上から下へ並べるので、行って戻る閉路があると、あとの節が先に来る（見本 293 では検証が精密化の上に、見本 299 では書店が出版社の上に出た）。**測った** —— **構成図 44 枚のうち閉路があるのは 8 枚**で、そこでは**逆向きに描かれる辺が 31%**、閉路の無い 36 枚では **15%** だった（2026-09-21 に測り直した。母数が 33 → 44 枚に増えても、差は 2 倍のまま）。戻りを描きたいなら、**終端の節にする**（「返品」という節を 1 つ置いて、そこで止める）か、戻る先を節の technology に書く。',
...
const en: Catalog = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**If your edges carry labels, try direction: down first** (structure diagrams; measured across the 25 examples that had edge labels, 2026-09-20 — **there are 38 as of 2026-09-21, and the conclusion has not been re-measured**). **When several edges converge on one node, right leaves no room for their labels** — across the 25, down never hid more labels (0 cases) and hid fewer in 2. But **right gives a smaller sheet in 23 of 25**, so if every label is drawn, right is fine. Use down for hierarchies (courts, org charts, utilities) and right for processes.',
      '**Groups make the sheet bigger** (structure diagrams; re-measured across the 21 examples that use them, 2026-09-21). **All 21 shrank when groups were removed** — to 32-94% of the area, median 51%. **None of the 21 had a crossing either way**, so containers do not cause crossings, and no clear relation showed up with edges that go back (rework, resubmission, division). Use containers to say that things exist at once (a VPC, a floor, a zone), and expect the paper to grow for each one — for a diagram meant to be projected, drop them or use fewer.',
      ...
      '**A back edge scrambles the order of a structure diagram.** The machine lays nodes out top to bottom, so a cycle puts later nodes above earlier ones. Re-measured 2026-09-21 across 44 structure diagrams: the 8 with a cycle draw **31%** of their edges backwards, the 36 without only **15%** — the gap held at 2x as the collection grew from 33 to 44. If you want to show something going back, make it a terminal node and stop there, or say it in the node technology.',
