...
const ja = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**views を置いたら、図の下に 130 px 以上空ける。** 図の下には通り芯の符号（90 px）と図の名前（116 px）が出るので、そこへ次の図や注記を置くと重なる。**数の検査は鳴らない** —— overlappingText は節の文字どうししか見ていないため（見本 129〜134 で 3 枚続けて踏んだ）。左右も同じで、符号は図の両側に出る（1 枚に 2 つ並べるなら 260 px は離す）。',
  ...
  validate: {
    ...
    nameCrowded: (id: string) =>
      `ノード "${id}" の名前は箱に入りきらず、外へ出した先も空いていません（**他の要素に重なる**か、**紙の縁で切れます**）。箱を大きくするか、文字を短くしてください（消してはいません —— 名前が消えるのは、重なるより悪いためです）。`,
...
const en: Catalog = {
  ...
  tools: {
    ...
    rules: [
      ...
      'Leave 130px or more below a view. Grid-line codes (90px) and the view title (116px) are drawn below it, so anything placed there collides — and the counters do not catch it, because overlappingText only looks at node text. The same applies sideways: codes appear on both sides, so leave 260px between two views placed side by side.',
  ...
  validate: {
    ...
    nameCrowded: (id: string) =>
      `The name on node "${id}" does not fit its box and there is no free room outside either: it either lands on something else or is cut off at the edge of the sheet. Make the box bigger or shorten the text. It is NOT dropped: losing a name is worse than an overlap.`,
    nameAdrift: (id: string, needs: number, has: number) =>
