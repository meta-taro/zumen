...
const en: Catalog = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**If your edges carry labels, try direction: down first** (structure diagrams; **re-measured across all 38 examples with edge labels, 2026-09-21**). **When several edges converge on one node, right leaves no room for their labels** — laying all 38 out both ways, **right loses 17 labels and down loses none**. Down never hid more (0 cases) and hid fewer in **9** drawings, up from 2 when this was measured on 25. But **right still gives a smaller sheet in 28 of 38 (74%)**, so if every label is drawn, right is fine — that advantage has narrowed from 92%. Use down for hierarchies (courts, org charts, utilities) and right for processes.',
