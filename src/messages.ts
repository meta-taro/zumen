...
const ja = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**`arrows` は「矢印を出すか」だけでなく、「辺を箱の上に描くか」の切り替えでもある。** `arrows: true` だと辺が箱より上、`false` だと下に敷かれる（`src/render.ts`）。**測った** —— 配置図で `arrows: true` の 183 枚のうち **125 枚（68%）は、既定の矢印を 1 本も出していない**。みんな上下の順のために書いている。記号を線の中まで描き込みたい図（TOPS 図の「中心まで引いた線」、回路の分岐点）では `arrows: true` にして、端は `ends: { from: none, to: none }` で消す。逆に、線を図そのものとして敷きたい図（路線図・型紙・伏図）は `arrows: false`。',
...
const en: Catalog = {
  ...
  tools: {
    ...
    rules: [
      ...
      '**`arrows` does two things: it draws the default arrowheads, and it decides whether edges are drawn above or below the boxes.** Measured: of 183 placement drawings with `arrows: true`, **125 (68%) never draw a single default arrowhead** — they set it for the stacking order. Use `arrows: true` plus `ends: { from: none, to: none }` when a line has to reach into the middle of a symbol; use `arrows: false` when the lines are the drawing itself (transit maps, patterns, framing plans).',
