 ...
 *
 * **2026-09-15 の訂正。** ここには長いあいだ
 * 「S1 暫定形式。これは保存形式（D2）ではない」と書いてあったが、
 * **D2 は決まっている** —— 正本は zumen 独自の YAML で、
 * 仕様は実装から分離して `spec/zumen-format-v1.md` に置いた。
 * この実装は**その仕様の実装のひとつ**であって、実験用ではない。
 *
 * 仕様が別にあることには意味がある ——
 * **別の実装が同じファイルを読み書きできる**なら、
 * この製品が終わっても図は読める（D2 の条件そのもの）。
 *
