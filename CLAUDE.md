...
- **`.claude/rules/UIワイヤーフレームの方針.md`** — UI は**完成デザインを作らない**。
  Figma の代替ではなく、**要件と実装の間の「構造の正本」**として扱う。
  レスポンシブの**構造変化**と User Flow ＋ Component 構造が主役。
- **`.claude/rules/周の回し方.md`** — **時間で区切って回すときの手順**。
  「何時まで自動で進めておいて」と渡されたときに読む。**push しない前提で CI が走らない**ので、
  `pnpm test` が唯一の門になる。終わったら `pnpm laps --digest` で 1 枚にして渡す。
- **`.claude/rules/専門図面の調査と実装方針.md`** — **何を描くかの方針**。
  「zumen で何が描けるか」ではなく「**世の中にどんな専門図面があるか**」から考える。
  汎用ダイアグラムを量産しない。**調べずに描かない**。
...
> 基本的には zumen は**レパートリーを増やしつつ、ブラッシュアップを繰り返し続ける**ことがメインです。
> **たまに人の操作で GUI の UX や MCP を拡充する**感で。

| | 何をするか | 誰が起点か |
|---|---|---|
| **主** | **レパートリーを増やす**（`.claude/rules/専門図面の調査と実装方針.md` の順序で） | AI |
| **主** | **ブラッシュアップを 1 つ以上**（`.claude/sample-ideas.md` の「やり方」§5） | AI |
| **主** | **描いた図を 1 枚は実物で見る**（ブラウザで開く。数の検査は「読めるか」しか見ていない） | AI |
| 従 | GUI の UX | **人が操作して気づいたとき** |
| 従 | MCP の拡充 | 同上 |

- **GUI と MCP は、AI が思いつきで広げない。**
  人が実際に触って出た不満が起点。AI 側は気づいたことを積んでおくだけ
- レパートリーは**調べてから描く。**「○○らしい絵」を量産しない
