...
**350 枚を [`examples/gallery/`](examples/gallery/) に置いてあります。**
...
開いている口は 15 個。`zumen_spec`（形式を教える）／**`zumen_examples`（同梱の見本 350 枚の目次と正本を返す）**／
`zumen_create`（**ゼロから作る。既にあれば失敗**）／
`zumen_propose`（**提案の `pins` は読まない**）／`zumen_inspect`（交差・重なり・**置けずに消えたラベル**・**投影で読める大きさか**・**人が見たか**・「9 割」を返す）ほか。

`zumen_spec` が渡すのは**書き方**だけです。**何を描くか**は `zumen_examples` が渡します ——
歯周チャート・木取り図・舞台の仕込図・中古車の査定図・継手と仕口・点字ブロックが
**実務でどう組まれているか**。一行は題名ではなく、**その図の決まりごと**を書いてあります。
...
`pnpm mcp` を立てておくと、**開いているデスクトップアプリと線で繋がります。**
エージェントの提案が**その場で画面に出て**、人がそこで採るかどうかを決められます。

```
人「この 2 つ、逆にして」
  → エージェントが zumen_live_read で**いま画面にある正本**を読む（保存前の手直しごと）
  → zumen_live_propose で提案を**画面へ出す**
  → 画面に差分が出る。人が「正本へ入れる」か「やめる」を押す
人「あー、そうじゃなくて、DB は下」
  → エージェントは人が採らなかったことを知っていて、次の案を出す
```

| 口 | 何が起きるか |
|---|---|
| `zumen_live_status` | 画面が繋がっているか・何を映しているか・**人がいまどれを選んでいるか** |
| `zumen_live_read` | **画面が持っている正本**（**保存前の手直しを含む**。ディスクより新しい） |
| `zumen_live_propose` | 提案を**画面へ出す**。`wait` を渡すと人の答えを待つ |
| `zumen_live_point` | 「この箱のことです」と指す（**選ぶだけ。図は変わりません**） |
| `zumen_timelapse` | **図が育つところを 1 本にする**（動く SVG ＋ 紙を揃えた連番。画面録画は要りません） |

**正本は変わりません。** 入れるのは人が押したとき。**線の向こうに押す口はありません。**
`applied` が返るのは人が押したときだけで、`timeout` は
**「まだ見ていない」であって断られたではありません。**

むしろ承認は強くなります。`zumen_propose` は**先にファイルを書く**ので、
人が見るのは書き換わった後の図でした。線を通すと、人が見るのは**入れる前の差分**です。
...
検査は `@metataro/zumen/tools` の `inspect`（交差・重なり・置けずに消えたラベル・「9 割」）。
...
**技術的な決定は D1〜D33 まで記録してあり、未決は D3（「9 割」の測り方）だけです**
...
**描けないものも 1 つ測りました**（2026-09-15）。
足関節のテーピング指示図を作ろうとして、**輪郭は描けたのにテープが描けませんでした。**
テープは「体に**巻きついた帯**」で、**線に幅を持たせる手段**と、
**裏へ回った部分を表す（線を切る）手段**が要ります。
太い線を 8 本重ねたら輪郭が埋もれました。**同じ壁はロープの結索図にもあります**
（[`.claude/sample-ideas.md`](.claude/sample-ideas.md) 課題 10）。
