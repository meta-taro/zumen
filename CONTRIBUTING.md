...
**この 3 つは、AI が無人で緑にできます。**「動くか」しか見ていないためです。
**人が画面を見たかどうか**は別に残します（`qa/README.md`）。

```bash
pnpm qa:verify   # git-qa の証跡を読み返す。証跡がまだ無ければ終了コード 2
```

`pnpm test` には入れていません。**人が走らせるまで落ち続ける関門**なので、
常時緑であるべき検査と混ぜると、どちらかを黙らせることになります。
