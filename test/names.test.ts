...
const CROSSINGS_ARE_CONTENT: Record<string, string> = {
  '352-網点で濃さを出す.zumen.yaml':
    '**うなり（モアレ）は、網が交わってできる。** ⑤は同じ間隔の線の網を 7° と 30° ずらして重ね、交わりの粗密がそのまま縞になることを見せている —— 交わらない網を重ねても、なぜ角度をずらすのかは描けない',
...
const LAYERED_ON_PURPOSE: Record<string, string> = {
  '352-網点で濃さを出す.zumen.yaml':
    '**網点は、濃いところで隣と繋がる。** 点の面積を濃さに合わせて決めているので、78.5% を超えた升目では丸が隣へはみ出す —— 繋がることがトーンジャンプの原因で、②の中身そのもの。離して描いたら、暗部が暗くならない',
